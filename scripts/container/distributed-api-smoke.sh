...
for name in "$postgres_name" "$rustfs_name" "$application_name"; do
...
for _ in $(seq 1 80); do
  if curl --fail --silent "http://127.0.0.1:$application_port/health/ready" \
    ...
    echo "Final-image distributed API smoke passed for $image."
    exit 0
  fi
  if [[ "$(podman inspect "$application_name" --format '{{.State.Running}}')" != true ]]; then
