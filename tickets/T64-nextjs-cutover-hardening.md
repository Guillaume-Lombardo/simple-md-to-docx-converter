...
* 2026-09-03: Independent review found cutover edge cases before merge. The correction compares image versions before their distinct digests in both quickstarts, enforces matched versions in the E2E harness, gives recovery Skopeo an isolated authenticated GHCR authfile, targets the shared-namespace router healthcheck on port 3100, and bounds every upstream relay with downstream-disconnect cleanup. The router now defers upstream headers until the complete request body passes its byte ceiling, so an early upstream response cannot override the exact empty `413`. Native final-image browser support again has a blocking coverage runner (100% lines/functions and 97.87% branches), the full frontend suite passes 186 tests at 90.58% branches, and 278 focused Python/Compose/CI tests pass. Hosted validation of the next exact head remains required.
