...
* 2026-08-30: CodeRabbit identified Uvicorn's nonzero `SystemExit` startup boundary, which bypassed the normal `Exception` sanitizer on bind failure. `serve` now maps only nonzero integer startup exits to the stable `runtime_failure` envelope while allowing clean or non-integer exits to propagate; human and JSON regressions cover the behavior. The exact unit command passes 1,476 tests.
* 2026-08-30: Verified complete on `main` at `6902f75`. PR #110 passed CI and CodeRabbit at exact head `38254b6`, squash-merged as `93688f7`, and its exact merge SHA passed main CI run `33309428939`.
