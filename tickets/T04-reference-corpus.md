...
- 2026-08-24: Added the manifest-validated reference corpus with all required categories and future owners; deterministic, provenance-pinned DOCX and adversarial ZIP builders; strict path/archive/OpenXML inspection; explicit RGBA raster tolerances and diagnostics; reusable fixtures; unit and integration coverage; planned document-engine CI selection; and usage documentation. T04 adds no conversion behavior or final-image workflow, so E2E coverage is not applicable. Linear remains `In Progress` until the change is verified on `main`.
