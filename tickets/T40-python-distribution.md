...
* 2026-08-30: Integrated the merged T47 changelog, upgrade guidance, and release-workflow validation from `main`. The T40 distribution and final-image entrypoint patch remained byte-for-byte unchanged; the 89-test `--no-cov` distribution suite, CI validator, Ruff, and `ty` all passed on the combined revision.
