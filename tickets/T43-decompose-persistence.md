...
* 2026-08-30: Validation passes Ruff, `ty`, 1,538 unit tests at 94.18% application coverage, the 90% branch gate, 94.43% changed-line coverage, 38 focused SQLite/contract tests, 31 real PostgreSQL 18/RustFS tests, and the 1,781-test canonical non-document-engine matrix at 95.94% coverage. Final-image service/security/conversion E2E passed for both profiles. The unrelated shared browser workflow remained nondeterministic after those passes: two standalone runs failed at different admin-session/template steps (`401`, then `422`) and one distributed run failed at a third admin-session step (`401`); local host document-engine executables are unavailable, so the bare full Pytest command was not run outside the final image.
