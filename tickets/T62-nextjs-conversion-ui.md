...
* 2026-09-02: Hosted standalone and distributed evidence showed the terminal PDF-limit job failed authoritatively while a newly opened secondary Chromium page emitted no status GET. The browser test now reuses and foregrounds Alice's existing workspace, requires the first post-acceptance status GET explicitly, and retains exact failed-state UI assertions; a fake-timer controller regression proves `Retry-After` polling publishes the terminal failure.
