...
* 2026-09-02: Hosted rerun `33662653155` advanced through account administration and exposed that FastAPI treated the frontend's exact empty multipart sentinel for `expected_fonts` as a literal invalid font during the second template replacement. Create and replace now share an exact decoder: the sole value `""` clears the declaration, while whitespace-only and mixed empty/non-empty API values remain unmodified and fail the existing font contract. Empty declarations still require every referenced DOCX font to resolve through the approved policy. Route, functional, validator, OpenAPI, frontend transport, browser, and final-image coverage preserve ordered non-empty declarations and explicit clearing; a hosted both-profile rerun remains required.
* 2026-09-02: Hosted rerun `33665521477` passed the preceding administration scenarios and exposed an incorrect E2E expectation for deleting a template that still owns the system-fallback reference. The documented and OpenAPI contract intentionally maps stale, malformed, and disallowed ETag-guarded template mutations to `412`, not duplicate-state `409`. The final-image journey now requires the authoritative `412`, verifies a fresh snapshot and no automatic replay, moves the fallback, then reopens the confirmation and retries through an explicit user click. A component regression locks the same refresh, dialog dismissal, and two-click behavior. The complete frontend check passes with 179 tests and unchanged coverage thresholds; build/runtime and focused backend contract, OpenAPI, documentation, harness, Ruff, ty, bindings, and syntax checks pass. A hosted both-profile rerun remains required.
* 2026-09-02: Hosted run `33666928584` stopped in the light frontend suite because the new delete-guard component regression synchronously inspected the dialog immediately after the error notice rendered. React had already cleared the confirmation state, but the shared dialog's effect had not yet removed its native `open` attribute, so the transient closing render exposed the fallback title `Archive template?`. The regression now waits for the complete absence of any dialog, preserving the exact DELETE-dialog closure requirement without changing product behavior or weakening selector scope. Review then reproduced the previously misclassified conversion-component failure under full coverage: the test could attempt the second download before the workspace's debounced initial template search, whose start clears the shared error state. The test now waits for that third initialization request before exercising downloads, leaving the exact visible-error assertion intact. Three consecutive canonical `npm run check` executions pass all 179 tests and every coverage threshold. The hosted final-image jobs from the failed head were stale and require a fresh run.
* 2026-09-02: Hosted run `33668267498` passed every preceding administration scenario, but both final-image profiles timed out after Bob's account deactivation because the E2E reloaded the page before checking the revoked-session notice. A reload creates a fresh authentication controller, whose unauthenticated bootstrap `401` intentionally cannot distinguish a revoked prior session from an ordinary signed-out visit and therefore shows no expiry notice. The corrected journey keeps Bob's hydrated session alive, submits one real sign-out request after revocation, requires its authoritative `401`, the fixed sign-in-again notice, and exactly one non-replayed request. This preserves the T61 revocation contract rather than weakening the user-visible assertion. The complete frontend check passes all 179 tests and coverage thresholds; production build/runtime, focused harness, OpenAPI/documentation/CI-selection tests, Node/Bash syntax, Ruff, ty, formatting, and diff checks also pass locally. A hosted both-profile rerun remains required.
