...
Version `0.3` is the first approved public release. The authoritative PEP 440 version is
`project.version` in `pyproject.toml`; the corresponding Git tag and GitHub Release tag are exactly
`v0.3`. Publication is triggered only when that GitHub Release is published. Creating or pushing a
...
verify that an anonymous client can pull the immutable `0.3` tag. Do not grant the package access to
unrelated repositories.

## Prepare and publish `v0.3`
...
version `0.3`, Apache-2.0 metadata, both wheel and sdist, and attestations. Confirm that the pending
...
Verify the GHCR `0.3` digest matches the workflow output, the provenance attestation verifies for
