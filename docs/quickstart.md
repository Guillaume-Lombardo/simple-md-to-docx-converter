...
Run all commands below from the repository root. For the short first-run path, see the
[README](../README.md). These profiles are for local evaluation, not production.
...
[container deployment guide](container-deployment.md), [resource policy](resource-policy.md),
[storage profiles](storage-profiles.md), and [authentication guide](authentication.md).
