...
| Python distribution | `markweave` on PyPI with public import `md_converter`; availability must be rechecked immediately before the first publication attempt, and a pending Trusted Publisher does not reserve the name |
...
Use an isolated release workflow to publish the `markweave` Python distribution while preserving the public import `md_converter`. Build its sdist and wheel once from the reviewed tagged source, then validate metadata, installation, the documented public import, and artifact integrity before publication. Publish those exact verified artifacts to PyPI; do not rebuild them in the publication job. Use PyPI Trusted Publishing through GitHub OIDC and the dedicated GitHub `pypi` environment as an identity boundary, with no long-lived PyPI token. The environment has no required reviewer or manual approval; publishing the GitHub Release is the sole human release gate. This deliberately trades a second approval for a simpler release path, so release-creation permissions and the trusted repository guard are security-critical. Grant `id-token: write` only to the minimal PyPI upload job within the isolated workflow; a separate provenance or attestation job owned by T22 may receive the same permission only when its need is documented and its remaining permissions are least privilege. Pin every action by full commit SHA, generate and upload PyPI publish attestations, and prevent pull requests, forks, and all other untrusted contexts from publishing. Version `0.3` maps to tag `v0.3`, and publication is triggered only by publishing that GitHub Release.

Before the first public release, configure a PyPI pending Trusted Publisher for project `markweave`, owner `Guillaume-Lombardo`, repository `simple-md-to-docx-converter`, workflow `release.yml`, and environment `pypi`. Immediately before the first publication attempt, recheck that `markweave` remains available and stop if it does not. A pending publisher does not reserve the distribution name. The first successful OIDC upload creates the PyPI project and converts the pending publisher into a normal Trusted Publisher; verify both the project and publisher state after that upload. The public license is Apache-2.0.
