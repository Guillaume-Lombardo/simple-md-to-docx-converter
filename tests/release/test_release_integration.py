...
def test_real_build_validation_clean_install_and_tamper_failure(
    ...
    verified = verify_release(
        output, expected_name="markweave", expected_version="0.3.5"
    )
    ...
    with zipfile.ZipFile(verified.wheel) as wheel:
        ...
        assert "markweave-0.3.5.dist-info/licenses/LICENSE" in names
    with tarfile.open(verified.sdist, mode="r:gz") as sdist:
        names = set(sdist.getnames())
        assert "markweave-0.3.5/LICENSE" in names
        assert "markweave-0.3.5/src/markweave/__init__.py" in names
    ...
    with pytest.raises(ArtifactError, match="integrity check failed"):
        verify_clean_install(
            tampered, expected_name="markweave", expected_version="0.3.5"
        )
